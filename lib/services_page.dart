...
class _ServicesPageState extends State<ServicesPage> {
  ...
  Widget _buildDynamicAds() {
    // إذا كانت الفئة "الكل" نعرض إعلانات عامة فقط
    // إذا كانت فئة محددة نعرض إعلانات عامة + إعلانات خاصة بالفئة
    if (_selectedCategory == "all") {
      ...
      // عرض إعلانات من مصدرين: العامة + الخاصة بالفئة
      return _ServicesAdsBanner(
        generalPageLocation: 'services',
        categoryPageLocation: 'services_$_selectedCategory',
        position: 'top',
      );
    }
  }
...
// Widget مخصص لجلب الإعلانات من مصدرين (عامة + خاصة بالفئة)
class _ServicesAdsBanner extends StatefulWidget {
  final String generalPageLocation;
  final String categoryPageLocation;
  final String position;
  
  const _ServicesAdsBanner({
    required this.generalPageLocation,
    required this.categoryPageLocation,
    required this.position,
  });
  
  @override
  State<_ServicesAdsBanner> createState() => _ServicesAdsBannerState();
}

class _ServicesAdsBannerState extends State<_ServicesAdsBanner> {
  List<dynamic> _ads = [];
  bool _loading = true;
  ...
  Future<void> _loadAds() async {
    try {
      print('📢 [SERVICES_ADS] Loading ads from ${widget.generalPageLocation} and ${widget.categoryPageLocation}...');
      
      // جلب الإعلانات العامة
      final generalResponse = await http.get(
        Uri.parse('https://dalma-api.onrender.com/api/ads/${widget.generalPageLocation}'),
      );
      
      // جلب الإعلانات الخاصة بالفئة
      final categoryResponse = await http.get(
        Uri.parse('https://dalma-api.onrender.com/api/ads/${widget.categoryPageLocation}'),
      );
      
      List<dynamic> allAds = [];
      
      if (generalResponse.statusCode == 200) {
        final List<dynamic> generalAds = jsonDecode(generalResponse.body);
        allAds.addAll(generalAds);
        print('✅ [SERVICES_ADS] Loaded ${generalAds.length} general ads');
      }
      
      if (categoryResponse.statusCode == 200) {
        final List<dynamic> categoryAds = jsonDecode(categoryResponse.body);
        allAds.addAll(categoryAds);
        print('✅ [SERVICES_ADS] Loaded ${categoryAds.length} category-specific ads');
      }
      
      // فلترة حسب الموضع وإزالة التكرار (بناءً على id)
      final Map<int, dynamic> uniqueAds = {};
      for (var ad in allAds) {
        if (ad['position'] == widget.position) {
          uniqueAds[ad['id']] = ad;
        }
      }
      
      setState(() {
        _ads = uniqueAds.values.toList();
        _loading = false;
      });
      
      print('✅ [SERVICES_ADS] Total unique ads: ${_ads.length}');
      
      // Track impression for first ad
      if (_ads.isNotEmpty) {
        _trackImpression(_ads[0]['id']);
        _currentPage = 0;
        print('👁️ [SERVICES_ADS] Initial impression tracked for first ad (index 0, Ad #${_ads[0]['id']})');
      }
  ...
  Future<void> _trackImpression(int adId) async {
    if (_trackedImpressions.contains(adId)) {
      print('⏭️ [SERVICES_ADS] Ad #$adId already tracked, skipping');
      return;
    }
    try {
      print('👁️ [SERVICES_ADS] Tracking impression for ad #$adId');
      await http.post(
        Uri.parse('https://dalma-api.onrender.com/api/ads/$adId/impression'),
      );
      _trackedImpressions.add(adId);
      print('✅ [SERVICES_ADS] Impression successfully tracked for ad #$adId');
    } catch (e) {
      print('❌ [SERVICES_ADS] Impression tracking error for ad #$adId: $e');
    }
  }
  
  Future<void> _trackClick(int adId) async {
    try {
      print('🖱️ [SERVICES_ADS] Tracking click for ad #$adId');
      await http.post(
        Uri.parse('https://dalma-api.onrender.com/api/ads/$adId/click'),
      );
      print('✅ [SERVICES_ADS] Click successfully tracked for ad #$adId');
    } catch (e) {
      print('❌ [SERVICES_ADS] Click tracking error for ad #$adId: $e');
    }
  }
  
  void _handleAdTap(dynamic ad) async {
    final adId = ad['id'];
    ...
    if (linkType == 'internal') {
      ...
      if (linkUrl != null && linkUrl.toString().isNotEmpty) {
        try {
          ...
          print('❌ [SERVICES_ADS] Error launching URL: $e');
        }
      }
    }
  }
  ...
  void _navigateToInternalRoute(String route) {
    ...
    switch (route) {
      ...
      case '/realty':
        ...
        break;
      default:
        print('⚠️ [SERVICES_ADS] Unknown internal route: $route');
        return;
    }
        ...
        Container(
          ...
          child: PageView.builder(
            ...
            onPageChanged: (index) {
              ...
              print('📊 [SERVICES_ADS] Page changed to index $index (Ad #${ad['id']})');
            },
            itemBuilder: (context, index) {
              final ad = _ads[index];
              return GestureDetector(
