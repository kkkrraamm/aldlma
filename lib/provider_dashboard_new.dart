    ...
    if (query.isEmpty) {
      ...
      filteredProducts = widget.products
          .where((p) =>
              ...
              (p['description'] ?? '').toString().toLowerCase().contains(query.toLowerCase()))
          .toList();
    }
    setState(() {});
  }
          ...
          child: filteredProducts.isEmpty
              ? Center(
                  child: Column(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      Icon(Icons.search_off_rounded, size: 48, color: theme.textSecondaryColor),
                      const SizedBox(height: 16),
                      Text(
                        'لم نجد منتجات',
                        style: GoogleFonts.cairo(
...
class _VideosTabState extends State<_VideosTab> {
  late List<Map<String, dynamic>> filteredVideos;
  final searchController = TextEditingController();
    ...
    if (query.isEmpty) {
      ...
      filteredVideos = widget.videos
          .where((v) =>
              ...
              (v['description'] ?? '').toString().toLowerCase().contains(query.toLowerCase()))
          .toList();
    }
    setState(() {});
  }
  ...
  void _showDeleteVideoConfirmation(BuildContext context, Map<String, dynamic> video) {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          'هل أنت متأكد من رغبتك في حذف ${video['title']}؟',
          style: GoogleFonts.cairo(),
        ),
        actions: [
          TextButton(
